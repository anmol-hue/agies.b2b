...
export function PatientManager({
  ...
  const handleSubmitNewPatient = (e: React.FormEvent) => {
    ...
    const newPatient: Patient = {
      ...
      chiefComplaint: newPatComplaint.trim(),
      symptoms: newPatSymptoms.trim() ? newPatSymptoms.split(',').map(s => s.trim()).filter(Boolean) : [],
      allergies: newPatAllergies.trim() ? newPatAllergies.split(',').map(a => a.trim()).filter(Boolean) : [],
      vitals: {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase bg-blue-900/40 text-blue-300 border border-blue-500/30">
                <Building2 className="w-3.5 h-3.5 text-blue-400" />
                {user.doctorProfile?.hospital || 'Metropolitan General Hospital'}
              </span>
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase bg-slate-800 text-slate-300 border border-slate-700">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-100 flex items-center gap-2">
              ...
              <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-emerald-900/40 text-emerald-400 border border-emerald-500/30">
                Live B2B Station
              </span>
            </h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-xl shadow-sm shadow-blue-500/20 transition-all active:scale-[0.98]"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2.5 bg-slate-900/60 hover:bg-slate-800 text-slate-300 text-sm font-medium rounded-xl transition-all border border-slate-700"
            >
          ...
          <div className="flex items-center gap-1.5 clinical-surface-inner p-1.5 rounded-xl border border-blue-500/30">
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                selectedStatusTab === 'Pending' ? 'bg-amber-600 text-white' : 'bg-amber-900/40 text-amber-300'
              }`}>
                {pendingCount}
              </span>
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                selectedStatusTab === 'Consulted' ? 'bg-blue-700 text-white' : 'bg-blue-900/40 text-blue-300'
              }`}>
                {consultedCount}
              </span>
            </button>
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                selectedStatusTab === 'Treated' ? 'bg-emerald-700 text-white' : 'bg-emerald-900/40 text-emerald-300'
              }`}>
                {treatedCount}
              </span>
            </button>
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded-lg text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50"
              />
            </div>
            <select
              value={departmentFilter}
              onChange={(e) => setDepartmentFilter(e.target.value)}
              className="py-1.5 px-2.5 text-xs bg-slate-900 border border-slate-800 rounded-lg text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            >
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 border-b border-slate-800 text-xs font-bold">
                  {[
                    ...
                    { id: 'scans', label: `Scans & Labs (${(activePatient.attachments || []).length})`, icon: Paperclip },
                    { id: 'prescriptions', label: `Prescriptions (${activePatient.prescriptions.length})`, icon: Pill },
                    { id: 'soap', label: `SOAP Notes (${(activePatient.doctorNotes || []).length})`, icon: FileText }
                  ].map((tab) => {
                    const Icon = tab.icon;
              ...
              <div className="bg-rose-900/20 border border-rose-500/30 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="px-3.5 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs shrink-0 flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <Trash2 className="w-3.5 h-3.5" />
            ...
            >
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl transition-all shadow-sm cursor-pointer flex items-center gap-1.5"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Delete Patient</span>
                </button>
